...
impl IoCmd {
    ...
    fn run_bytes<T: trussed::Client + AuthClient>(
        ...
        let mut rep: heapless::Vec<u8, 7609> = heapless::Vec::new();
        let cmd: iso7816::Command<7609> = iso7816::Command::try_from(input).unwrap_or_else(|err| {
    ...
    fn run_get_data<T: trussed::Client + AuthClient>(
        data_object: DataObject,
        occurence: &Option<Occurence>,
        expected_data: &[u8],
        expected_status: Status,
    ...
    fn run_select_data<T: trussed::Client + AuthClient>(
        data_object: DataObject,
        occurence: Occurence,
        expected_status: Status,
        card: &mut opcard::Card<T>,
